...
resource "aws_sns_topic" "gogotopic" {
  ...
  provisioner "local-exec" {
    command = "export AWS_ACCESS_KEY_ID="DFFJDUDDN3843j" ; export AWS_SECRET_ACCESS_KEY=${var.secret_key}; aws sns subscribe --topic-arn ${aws_sns_topic.gogotopic.arn} --protocol email --notification-endpoint ${var.emails} --region ${var.region}"
  }
}
